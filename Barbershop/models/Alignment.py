...
class Alignment(nn.Module):
    def __init__(self, tool):
        ...
        self.tool = tool
        self.loss_builder = AlignLossBuilder(tool.opts)
    ...
    def create_target_segmentation_mask(
        ...
        down_seg1, _, _ = self.tool.seg(im1)
        ...
        down_seg2, _, _ = self.tool.seg(im2)
        ...
        #     save_vis_mask(img_path1, img_path2, sign, self.tool.opts.output_dir, target_mask.squeeze().cpu())
    ...
    def preprocess_img(self, img_path):
        im = (
            ...
            .unsqueeze(0)
            .to(self.tool.opts.device)
        )
        im = (self.tool.downsample(im).clamp(0, 1) - seg_mean) / seg_std
        return im
    ...
    def setup_align_optimizer(self, latent_path=None):
        if latent_path:
            ...
            latent_W = (
                self.tool.net.latent_avg.reshape(1, 1, 512)
                .repeat(1, 18, 1)
                .clone()
                .detach()
                .to(self.tool.opts.device)
        ...
        optimizer_align = opt_dict[self.tool.opts.opt_name](
            [latent_W], lr=self.tool.opts.learning_rate
        )
    ...
    def create_down_seg(self, latent_in):
        ...
        im = (self.tool.downsample(gen_im_0_1) - seg_mean) / seg_std
    ...
    def align_images(
        ...
        intermediate_align, _ = self.tool.net.generator(
            [latent_in],
            input_is_latent=True,
            return_latents=False,
            start_layer=0,
        ...
        for step in range(self.tool.opts.align_steps2):
            ...
            #### Style Loss
            H1_region = self.tool.downsample_256(I_Structure_Style_changed) * HM_Structure
            H2_region = self.tool.downsample_256(gen_im) * HM_G
        ...
        latent_F_out_new, _ = self.tool.net.generator(
            [latent_in],
            input_is_latent=True,
            return_latents=False,
            start_layer=0,

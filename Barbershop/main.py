...
def main(args):
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ------ Step 1: Invert images")
    im_set = {im_path1, im_path2, im_path3}
    ii2s.invert_images_in_W([*im_set])
    ii2s.invert_images_in_FS([*im_set])

    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] ------ Step 2: Create mask and align images"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--template_dir", type=str, default="template", help="The template directory"
    )
